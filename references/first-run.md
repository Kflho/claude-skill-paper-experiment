...
> 科研 skill 基于**五文件工作流**管理你的研究：
>
> | 文件 | 内容 | 谁维护 |
> |------|------|--------|
> | **{项目名} note** | 目标、流程、结论 | 你 |
> | **{项目名} src** | 代码架构、数据管线、设计决策、参数溯源 | 你 + AI 可写 |
> | **{项目名} report** | 每个实验一个 H3 块（日期、结论、参数、指标） | AI 写入 |
> | **{项目名} 参考文献** | 论文解读与方法论笔记 | 你 |
> | **{项目名} schedule** | 每日任务进度（`# MM.DD` 格式）+ 问题跟踪（`# 问题`） | 你（AI 只可勾选/取消已有复选框，禁止新增/编辑任何文字） |
>
> AI 每次回复会标注目标文档和章节（如 **→ {项目名} note / 目标**），你直接复制粘贴到 Obsidian。
>
...
> | 2 | **项目名称** | 如 `pg 04`（Obsidian 文件名用小写+空格，不用下划线） | ✅ 必需 |
> | 3 | **论文文件夹路径** | 存放论文 PDF/markdown 的目录（可在项目内，如 `references/`） | ✅ 必需 |
> | 4 | **LaTeX 文件夹路径** | 存放 `v1.0.tex`、`.latexmkrc` 的目录 | 写论文时必需 |
> | 5 | **MATLAB 项目根目录** | 实验脚本所在（含 `src/main/`、`src/lib/` 等） | 写仿真章节时必需 |
>
> 你的五文件叫什么名字？（默认：`{项目名} note.md`、`{项目名} src.md`、`{项目名} report.md`、`{项目名} 参考文献.md`、`{项目名} schedule.md`）
>
> - 用默认命名 → 直接确认
> - 自定义名称 → 逐个告诉我
...
> - src: `{vault}/{项目名} src.md`
> - report: `{vault}/{项目名} report.md`
...
>
> - **y** → 创建含默认 H1 的文件（note=`# 目标`，src=`# 架构`，report=`# 实验报告`，参考文献=`# 论文解读`，schedule=`# MM.DD`）
> - **n** → 记录路径，标注 `(pending)`
