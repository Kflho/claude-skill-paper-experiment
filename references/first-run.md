...
> | 文件 | 内容 | 谁维护 |
> |------|------|--------|
> | **Project XX Note** | 目标、流程、结论 | 你 |
> | **Project XX src** | 代码架构、数据管线、设计决策、参数溯源 | 你 + AI 可写 |
> | **Project XX report** | 每个实验一个 H3 块（日期、结论、参数、指标） | AI 写入 |
> | **Project XX 参考文献** | 论文解读与方法论笔记 | 你 |
> | **Project XX Schedule** | 每日任务进度（`# MM.DD` 格式）+ 问题跟踪（`# 问题`） | 你（AI 只可勾选/取消已有复选框，禁止新增/编辑任何文字） |
>
> AI 每次回复会标注目标文档和章节（如 **→ Project XX Note / 目标**），你直接复制粘贴到 Obsidian。
>
