...
四个 H1 段：`# scripts`、`# lib`、`# tests`、`# main`（lib 内 H2 按阶段分组）。核心要求：**每项标注上下游依赖，读文档即可落地。**

**src.md 写作必检**（写/更新时必须逐条满足；旧内容不满足先重排，禁止保留平铺函数目录）：
1. 固定四段结构，lib 按论文推导链/流水线阶段分组。
2. 每项函数/脚本/测试/实验含字段：功能 + 输入 + 输出 + 下游（`→` 指向调用方）。
