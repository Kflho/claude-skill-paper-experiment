...
| 3 | **实验报告** | Note | `## 实验报告` | 每个实验的完整报告（日期、结论、参数、指标、备注） |
| 4 | **实验脚本** | MATLAB | `src/main/experiment_XX_*.m` | 注释中的目标说明、方法描述、验收标准 |
| 5 | **项目日程表** | Schedule | 对应日期标题下 | 任务复选框、完成状态、结果摘要 |

**同步触发规则：**
- 跑完实验 → 更新 ①②③④⑤（脚本注释如无需改动可跳过④）
- 修正实验逻辑 → 更新 ①②③④⑤
- 修改验收标准 → 更新 ①②③④⑤
- 更新实验结论 → 更新 ①③⑤
...
**同步检查方法：**
1. 检查目标行是否包含最新结果
2. 检查流程复选框是否标记完成、结果摘要是否与报告一致
3. 检查实验报告结论、指标数值是否与脚本实际输出一致
4. 检查脚本注释中的目标说明是否与流程中一致
5. 检查日程表中对应日期下任务复选框是否已标记
...
| 3 | **Note** | `# 架构` → `## scripts` / `## lib` / `## tests` / `## main` | 标记对应脚本复选框 `[x]`；若文件名与架构中不一致，更正文件名 |
