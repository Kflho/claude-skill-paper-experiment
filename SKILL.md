...
| Code review | 审查代码, review, 检查脚本, 简化 | src | `code-review`, `simplify` | `code-review` 双轴审查（规范 Standards + 逻辑 Spec）**+ 论文符合性轴**：判定标准/门限/归一化来自论文公式或已标注「论文未指定，实现选择」；论文方法无法实施的步骤已如实报告而非静默绕过；理论级不适用（铁律 #6）已上报。`simplify` 消除重复与冗余。 |
